...
class FumbleAward extends BaseAward {
    constructor(title, displayOrder = 1) {
        super(title, "Awarded to the manager whose team fumbled (lost) the most during the season.");
        this.title = title;
        this.id = "most-fumble-award";
        this.insights = [
          {
            key: "most_fumble_top",
            award_id: this.id,
            direction: "top",
            title: "Fumble Lost = Butterfingers",
            weekly_description: "{team_name} couldn't hold onto the ball this week.",
            season_description: "The team that coughed up the most fumbles all season.",
            icon: "🧈"
          }
        ];
        ...
        // * NOTE: In theory we can evolve this to get other turnover stats in the future.
        this.tdStatAbbr = "Lost";
    }

    build() {
        return new FumbleAward(this.title, this.displayOrder).setId(this.id).setDataSet(this.result).setProvider(this.provider).setLeagueId(this.leagueId);
    }
